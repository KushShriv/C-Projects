...
{ 
	...
	while (val--) { 
		if (alt == 0) { 
			iterLR--; 
			if (iterLR == player1) printf("#P1 "); 
			else if (iterLR == player2) printf("#P2 "); 
			...
			iterRL++; 
			if (iterRL == player1) printf("#P1 ");
			else if (iterRL == player2) printf("#P2 ");
...
{ 
	...
	for (int i = 0; i <= 100; i++) snakesAndLadders[i] = 0;
	...
	if (newSquare > 100) return currentPlayer;
	return newSquare; 
} 
